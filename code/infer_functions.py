...
def create_tensor_dataset(mode:str, lookbehind:int, limit = None):
    """
    ...
    if limit == None:
        limit = len(iterator)
    atemporal_columns = ["dividend", "splits", "category"]
    ...
    for filename in iterator[:limit]: #Iterates through the frames in the directory
        ...
        frame_tensor = frame_tensor.view(-1, len(sorted_frame.columns)//lookbehind, lookbehind) #shape before = (rows, columns) | shape after = (rows, columns/lookbehind,lookbehind)
